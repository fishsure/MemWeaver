...
class Seq2SeqDataset(Dataset):
    ...
    def get_prompt(self, inp, profile, summary=None):
        return self.get_prompt_fn(inp, profile, self.max_length, self.llm_tokenizer, summary=summary)

    def __getitem__(self, index):
        user_id = self.data[index]['user_id']
        all_profile = self.data[index]['retrieval']
        summary = self.data[index].get('summary', None)
        return {
            "user_id": user_id,
            "input": self.get_prompt(self.data[index]['input'], all_profile, summary=summary),
