...
class Retriever:
    ...
    def retrieve_topk_one_user(self, corpus, query, profile, user, topk):
        if self.ret_type == "bm25":
            ...
            original_rank = list(zip(profile, scores))
            sorted_idx = np.argsort(scores)[::-1][:topk]
            top_n_scores = [scores[i] for i in sorted_idx]
            selected_profs = [profile[i] for i in sorted_idx]

            # save debug info
            if len(self.bm25_debug_log) < 10:
                before = [{"text": profile[i], "score": float(scores[i])} for i in range(min(5, len(profile)))]
                after = [{"text": profile[i], "score": float(scores[i])} for i in sorted_idx[:5]]
