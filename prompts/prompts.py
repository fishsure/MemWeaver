...
# LaMP_1
def get_prompt_LaMP_1(inp, profile, max_length, tokenizer, summary=None):
    ...
    summary_prefix = f"User summary: {summary}\n" if summary else ""

    instruction = ""
    if len(profile) == 0:
        instruction = """Please choose one of the following two references that is more relevant to the user's input title.\n"""\
            f"""[1] {refs[1]}\n[2] {refs[2]}\n"""\
        ...
        per_p_max_length = max_len_prompt // len(profile) if profile else 0
        prompts = []
        saved_tokens = 0
        for p in profile:
    ...
    return summary_prefix + instruction


# LaMP_2
def get_prompt_LaMP_2(inp, profile, max_length, tokenizer, summary=None):
    query = get_query_LaMP_2(inp)
    summary_prefix = f"User summary: {summary}\n" if summary else ""

    instruction = ""
    if len(profile) == 0:
        instruction = """Please select the tag from """\
...
def get_prompt_LaMP_3(inp, profile, max_length, tokenizer, summary=None):
    ...
    instruction = ""
    if len(profile) == 0:
        instruction = """What is the score of the following review on a scale of 1 to 5? just answer with 1, 2, 3, 4, or 5 without further explanation.\n"""\
                f"""{query} 'score':\n"""\
        ...
        for p in profile:
            ...
            try:
                ...
                print(f"Error Text: {p['text']} type: {type(p['text'])}")
                print(query)
                print(max_length, length_add, per_p_max_length, saved_tokens)
                print(f"max len: {per_p_max_length + saved_tokens - needed_part_len}")
                exit()
            saved_tokens += per_p_max_length - len(
                tokens['input_ids']) - needed_part_len
            new_text = tokenizer.batch_decode([tokens['input_ids']],
...
# LaMP_4
def get_prompt_LaMP_4(inp, profile, max_length, tokenizer, summary=None):
    query = get_query_LaMP_4(inp)
    summary_prefix = f"User summary: {summary}\n" if summary else ""

    instruction = ""
    if len(profile) == 0:
        instruction = """Please generate a title for the given user's input text. """\
...
# LaMP_5
def get_prompt_LaMP_5(inp, profile, max_length, tokenizer, summary=None):
    query = get_query_LaMP_5(inp)
    summary_prefix = f"User summary: {summary}\n" if summary else ""

    instruction = ""
    if len(profile) == 0:
        instruction = """Please generate a title for the given user's input abstract. """\
            """Please generate it in the following format: {'title': 'generated title'} without explanation, and use only English.\n"""\
                f"""{query} 'title':"""
    else:
        instruction = "The historical profiles are as follows:\n{{profile}}\n"\
...
# LaMP_7
def get_prompt_LaMP_7(inp, profile, max_length, tokenizer, summary=None):
    query = get_query_LaMP_7(inp)
    summary_prefix = f"User summary: {summary}\n" if summary else ""

    instruction = ""
    if len(profile) == 0:
        instruction = """Please paraphrase the user's input tweet without any explanation before or after it.\n"""\
